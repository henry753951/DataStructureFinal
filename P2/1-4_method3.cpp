...
struct OptionData {
    ...
    void show() {
        std::cout << tradeDate << " / " << productCode << " / " << strikePrice << " / " << expirationDate << " / " << callPut << " / " << tradeTime << " / " << tradePrice << " / " << tradeQuantity << " / " << openingPrice << endl;
    }
...
int main() {
    ...
    for (int i = 0; i < 4000000; i++) {
        ...
        for (int j = 0; j < count; j++) {
            ...
            if ((OD_1.productCode == OD_2.productCode) && (OD_1.strikePrice == OD_2.strikePrice) && (OD_1.expirationDate == OD_2.expirationDate) && (OD_1.callPut == OD_2.callPut)) {
                fail = 1;
                break;
            }
        }
